...
<body>
    <div class="container">
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $checkQuantityQuery = "SELECT QUANTITY FROM stocks WHERE STOCKS_ID = $stocksId";
            ...
            if ($checkQuantityResult && $row = mysqli_fetch_assoc($checkQuantityResult)) {
                ...
                if ($availableQuantity >= $quantity) {
                    ...
                    if (move_uploaded_file($_FILES["document"]["tmp_name"], $documentPath)) {
                        // Retrieve price from stocks table
                        $query = "SELECT PRICE FROM stocks WHERE STOCKS_ID = $stocksId";
                        ...
                        if ($result && $row = mysqli_fetch_assoc($result)) {
                            ...
                            $insertQuery = "INSERT INTO printingorder (STOCKS_ID, USER_ID, QUANTITY, COMMENT, TOTAL_PRICE, DOCUMENT) VALUES ($stocksId, $user_id, $quantity, '$comment', $totalPrice, '$documentPath')";
                            if (mysqli_query($conn, $insertQuery)) {
                                ...
                                $errorMessage = "Error inserting into printingorder table: " . mysqli_error($conn);
                                trigger_error($errorMessage, E_USER_ERROR);
                                echo "<p class='error-message'>Sorry, something went wrong. Please try again later.</p>";
                            }
                        } else {
                            echo "<p class='error-message'>Invalid selection. Please try again.</p>";
                        }
                    } else {
